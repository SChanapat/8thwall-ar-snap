...
const initMediaPreview = (options = {}) => {
  ...
  // for langauage switch on version 1.1
  ...
  if (navigator.canShare && navigator.canShare(shareTestObj)) {
    ...
    // if (languageCheck.innerHTML === 'English') {
    // actionButtonText.textContent = options.actionButtonShareText || 'Share'
    // }
    // } else if (languageCheck.innerHTML === 'ภาษาไทย') {
    //   actionButtonText.textContent = options.actionButtonShareText || 'แชร์'
    // }
  ...
  document.getElementById('secondaryButton').addEventListener('click', closePreview)
